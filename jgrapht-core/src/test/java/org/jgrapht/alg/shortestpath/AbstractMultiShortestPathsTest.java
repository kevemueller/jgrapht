...
/**
 * Abstract base class for JUnit tests of k-shortest path algorithms. Tests are
 * performed against these algorithms:
 * <ul>
...
public abstract class AbstractMultiShortestPathsTest<V, E> {
	...
	protected static enum GraphProperties {
		LOOP, MULTIEDGE, NEGATIVE_WEIGHT;
	}
	...
	protected static EnumSet<GraphProperties> GRAPH_WITH_MULTIEDGE = EnumSet.of(GraphProperties.MULTIEDGE);
	protected static EnumSet<GraphProperties> GRAPH_WITH_NEGATIVE_WEIGHT = EnumSet.of(GraphProperties.NEGATIVE_WEIGHT);
	protected static EnumSet<GraphProperties> GRAPH_WITH_LOOP_AND_MULTIEDGE = EnumSet.of(GraphProperties.LOOP,
			GraphProperties.MULTIEDGE);
	...
	protected final static class AlgorithmInfo<V, E> {
		...
		public AlgorithmInfo(String name, EnumSet<GraphProperties> algIncompatibility,
	 ...
	 *            the friendly name
	 * @param kspFactory
	 *            the factory returning the kshortestpath implementation to use.
	 * @param fVertexValue
	 *            the function converting a vertex from string to V
	 * @param graph
	 *            the graph
	 * @param k
	 *            the number of shortest paths to retrieve
	 * @param source
	 *            the source/from/start vertex
	 * @param sink
	 *            the sink/to/end vertex
	 * @param expectedKShortestPaths
	 *            the expected/reference list of k shortest paths
	 * @param expectedKShortestWeights
	 *            the expected/reference list of k shortest path's weights
	 */
	public AbstractMultiShortestPathsTest(String name, AlgorithmInfo<V, E> alg, Function<String, V> fVertexValue,
			Graph<V, E> graph, EnumSet<GraphProperties> graphProperties, int k, V source, V sink,
	...
	public void testBasics() {
		...
		for (GraphPath<V, E> p : paths) {
			...
			double edgesWeight = p.getEdgeList().stream().map(graph::getEdgeWeight).reduce(0.0, Double::sum);
...
//		System.out.println(paths);
	...
	/**
	 * Helper to create the cartesian product of graphs cross algs by mixing in
	 * the algs at the right place.
	 * 
	 * @param graphs
	 * @param algs
	 * @return
	...
	protected static <V, E> Collection<Object[]> blendGraphsAlgs(List<Object[]> graphs) {
		ArrayList<AbstractMultiShortestPathsTest.AlgorithmInfo<V, E>> algs = new ArrayList<AbstractMultiShortestPathsTest.AlgorithmInfo<V, E>>();
		algs.add(new AbstractMultiShortestPathsTest.AlgorithmInfo<V, E>("KShortestPaths", GRAPH_NONE,
				KShortestPaths<V, E>::new));
		algs.add(new AbstractMultiShortestPathsTest.AlgorithmInfo<V, E>("YenKShortestPaths+Dijkstra",
				EnumSet.of(GraphProperties.LOOP, GraphProperties.NEGATIVE_WEIGHT), YenKShortestPaths<V, E>::new));
		algs.add(new AbstractMultiShortestPathsTest.AlgorithmInfo<V, E>("YenKShortestPaths+BellmanFord",
				EnumSet.of(GraphProperties.LOOP),
	...
	/**
	 * Helper to compare k-shortest paths reference with actual returned values.
	 * Asserts that
	 * <ol>
	 ...
	 * </ol>
	 * The comparison allows paths of the same length to be returned in
	 * arbitrary order.
	 * 
	 * @param expectedPaths
	 *            reference paths
	 ...
	 *            actual paths.
	 */
	private static <V, E> void assertKPathsEquals(List<GraphPath<V, E>> expectedPaths, List<GraphPath<V, E>> paths) {
		...
		for (int i = 0; i < paths.size(); i++) {
			assertEquals("path weight mismatch @" + i, expectedPaths.get(i).getWeight(), paths.get(i).getWeight(), 0.0);
		}

		{
			// ensure we got the right paths by looking at the vertexlists
			List<List<V>> expectedVertexLists = expectedPaths.stream().map(GraphPath::getVertexList)
					.collect(Collectors.toList());
			List<List<V>> pathsVertexLists = paths.stream().map(GraphPath::getVertexList).collect(Collectors.toList());
			...
			assertTrue("additional paths were found: " + add2, expectedVertexLists.containsAll(pathsVertexLists));
			assertTrue("not all expected paths were found: " + add1, pathsVertexLists.containsAll(expectedVertexLists));
		}

		{
			// ensure we got the right paths by looking at the edgelists
			List<List<E>> expectedEdgeLists = expectedPaths.stream().map(GraphPath::getEdgeList)
					.collect(Collectors.toList());
			List<List<E>> pathsEdgeLists = paths.stream().map(GraphPath::getEdgeList).collect(Collectors.toList());
			...
			assertTrue("additional paths were found: " + add2, expectedEdgeLists.containsAll(pathsEdgeLists));
			assertTrue("not all expected paths were found: " + add1, pathsEdgeLists.containsAll(expectedEdgeLists));
		}
	}

	/**
	 * Helper to transform a string representation of a GraphPath into a
	 * GraphPath. An element of edgesOrNodes is either matching a from:to/weight
	 * in parentheses or is a node.
	 * 
	 * @param graph
	 *            the underlying graph
	 * @param edgesOrNodes
	 *            the array of edges or node strings
	 * @param weight
	 *            the weight
	 * @return a GraphPath
	 */
	private static <V, E> GraphPath<V, E> edgeOrNodeListToPath(Graph<V, E> graph, String[] edgesOrNodes, double weight,
			Function<String, V> fValueOf) {
		...
		for (int i = 0; i < edgesOrNodes.length; i++) {
			if ('(' == edgesOrNodes[i].charAt(0)) {
				...
				if (null != current && !current.equals(v0)) {
					throw new IllegalArgumentException("edge not from previous node" + edgesOrNodes[i]);
				}
				current = v0;
				if (null == from) {
					from = current;
